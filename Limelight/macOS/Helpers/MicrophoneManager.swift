    ...
    private func normalizedAuthorizationError(_ message: String) -> String {
        ...
        guard !trimmedMessage.isEmpty else {
            return isSandboxedBuild
                ? "No administrator authorization response was received. This build may have blocked the system authorization prompt."
                : "No administrator authorization response was received. Please try again."
        }
        ...
        if trimmedMessage.localizedCaseInsensitiveContains("timed out") {
            return "Administrator authorization timed out. Please try again."
        }

        if trimmedMessage.contains("(-60005)") {
            return isSandboxedBuild
                ? "The administrator authorization prompt did not appear. This build may be blocking the request."
                : "Administrator authorization did not complete. Check that this account has administrator access and try again."
        }
